...
在 `~/.config/lazydeck/init.lua` 中配置：

```lua
{
  dir = 'plugins/aria2.lazydeck',
  config = function()
    require('aria2').setup {
...
- `download_dir` 可指定默认下载目录；为 `nil` 时使用启动 `lazydeck` 时的当前工作目录
- 如果设置了 `rpc_secret`，启动时会自动附加 `--rpc-secret`
- 可通过 `auto_start = false` 关闭
- 可通过 `start_cmd = {...}` 覆盖默认启动命令
- 如果本地 RPC 连不上且 `aria2c` 不在 `PATH` 中，插件会直接提示这一点，而不是只显示通用连接错误
